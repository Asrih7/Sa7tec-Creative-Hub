...
export function PublicLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <div className="site-shell s7-shell">
            ...
            <header className={`s7-header ${scrolled ? "is-scrolled" : ""}`}>
                <div className="s7-header-inner">
                    ...
                    <nav className="s7-nav desktop-nav" aria-label="Primary navigation">
                        {links.map((link) => (
                            <button key={link.id} onClick={() => scrollToSection(link.id, location, navigate)} className="s7-nav-link">
                                {link.label}
                            </button>
                        ))}
                        <Link href="/contact" className="s7-nav-link s7-nav-cta">
                            {t("nav.contact")}
                        </Link>
                    </nav>
                    ...
                    <div className="s7-header-actions">
                        <div className="s7-desktop-actions desktop-nav">
                            ...
                            <button onClick={toggleTheme} aria-label={themeLabel} title={themeLabel} className="s7-theme-toggle">
                        ...
                        <div className="s7-mobile-actions mobile-nav">
                            ...
                            <button onClick={() => setMenuOpen((value) => !value)} aria-label="Toggle menu" aria-expanded={menuOpen} className="s7-menu-toggle">
                    ...
                    >
                        <nav className="s7-drawer-nav" aria-label="Mobile navigation">
                            {links.map((link, index) => (
                                <button key={link.id} onClick={() => scrollToSection(link.id, location, navigate)} className="s7-drawer-link">
                                    <span>0{index + 1}</span>
