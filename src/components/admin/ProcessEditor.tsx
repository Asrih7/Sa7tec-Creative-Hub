...
type Step = SiteContent["processSteps"][number];

export function ProcessEditor() {
  const { content, updateContent } = useContent();
  const { t } = useLanguage();
  const { toast } = useToast();
  const [items, setItems] = useState<Step[]>(content.processSteps);
  ...
  const add = () =>
    setItems((prev) => [
      ...prev,
      {
        ...
        title: { en: "New step", fr: "Nouvelle étape", ar: "خطوة جديدة" },
        description: { en: "Describe this step.", fr: "Décrivez cette étape.", ar: "صف هذه الخطوة." },
      },
    ]);

  const save = () => {
    updateContent({ processSteps: items });
  ...
  return (
    <Card className="bg-zinc-900 border-zinc-800">
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          <CardTitle>{t("admin.process_card")}</CardTitle>
          <CardDescription>{t("admin.process_card_sub")}</CardDescription>
        </div>
